...
class myFOSSIL_Specimen_Public
{
    ...
    public function bp_get_activity_content_body( $content )
    {
        ...
        case TimeInterval::POST_TYPE:
            ...
            break;
        case FossilGeochronology::POST_TYPE: //i am suspicious that this is never the case any more and TimeInterval now handles this -MHJ0602
        ...
        case Fossil::POST_TYPE:
            ...
            /*
            if (count ( $changes ) > 0) {
                foreach ($changes as $current) {
                    switch ( $current->key ) {
                        case "dimension_id":
                            return $current->key." => ".$current->to;
                            break;
    ...
    }public function add_rewrite_tags()
    {
        ...
        add_rewrite_tag( '%fossil_view%', '(information|history|discussion|images|settings)' );
    ...
    {
        add_rewrite_rule(
            '^fossils/([0-9]+)/(information|history|discussion|images|settings)/?',
